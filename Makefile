...
LDFLAG_LOCATION := github.com/metal-toolbox/disko/internal/version
...
DOCKER_IMAGE  := "ghcr.io/metal-toolbox/disko"
REPO := "https://github.com/metal-toolbox/disko.git"
...
ifeq ($(GO_VERSION), 0)
	...
	GOOS=linux GOARCH=amd64 go build -o disko \
	   -ldflags \
		"-X $(LDFLAG_LOCATION).GitCommit=$(GIT_COMMIT) \
         -X $(LDFLAG_LOCATION).GitBranch=$(GIT_BRANCH) \
         -X $(LDFLAG_LOCATION).GitSummary=$(GIT_SUMMARY) \
...
## build docker image and tag as ghcr.io/metal-toolbox/disko:latest
...
push-image-devel: build-image
	docker tag ${DOCKER_IMAGE}:latest localhost:5001/disko:latest
	docker push localhost:5001/disko:latest
	kind load docker-image localhost:5001/disko:latest
