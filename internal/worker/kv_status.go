...
var (
	virtualMediaStatusKVBucket = string(rctypes.VirtualMediaMount)

	defaultKVOpts = []kv.Option{
		kv.WithDescription("disko condition status tracking"),
...
func createOrBindKVBucketWithOpts(s events.Stream, opts *events.NatsKVOptions) (nats.KeyValue, error) {
	...
	return kv.CreateOrBindKVBucket(js, virtualMediaStatusKVBucket, kvOptions...)
}
