...
var (
	...
	errMount          = errors.New("error mounting image on BMC")
)
...
func (w *Worker) Run(ctx context.Context) {
	...
	if _, err := createOrBindKVBucketWithOpts(w.stream, w.cfg.NatsOptions.KV); err != nil {
		w.logger.WithError(err).Error("failed to create/bind to status kv" + virtualMediaStatusKVBucket)
	}
...
func (w *Worker) doWork(ctx context.Context, condition *rctypes.Condition, e events.Message) {
	...
	task.Status = "Mouting image on device"
	...
	case errAssetNotFound, errMount:
		// asset was not found
		task.SetState(rctypes.Failed)
		task.Status = err.Error()
...
func (w *Worker) runTaskWithMonitor(ctx context.Context, task *Task, e events.Message) error {
	...
	errInventoryQuery := errors.New("error querying inventory for asset")

	// fetch asset inventory from inventory store
	asset, err := w.repository.AssetByID(ctx, task.Parameters.AssetID.String())
	if err != nil {
	...
	switch task.Parameters.MountMethod {
	case rctypes.MountMethodURL:
		return w.mountURL(taskCtx, task, asset, doneCh)
	case rctypes.MountMethodUpload:
		return w.uploadImage(taskCtx, task, asset, doneCh)
	}

	// stop monitor routine
	close(doneCh)

	return errors.Wrap(errTaskFirmwareParam, "invalid image mount method: "+string(task.Parameters.MountMethod))
}

func (w *Worker) uploadImage(ctx context.Context, task *Task, asset *model.Asset, doneCh chan<- struct{}) error {
	ctx, span := otel.Tracer(pkgName).Start(
		ctx,
		"worker.uploadImage",
	)
...
func (w *Worker) mountURL(ctx context.Context, task *Task, asset *model.Asset, doneCh chan<- struct{}) error {
	ctx, span := otel.Tracer(pkgName).Start(
		ctx,
		"worker.mountURL",
	)
